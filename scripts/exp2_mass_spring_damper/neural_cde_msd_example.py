...
class Config:
    ...
    # Forcing parameters
    forcing_type = 'pink_noise'  # Using advanced forcing from msd_simulation_with_forcing
...
class Func(eqx.Module):
    ...
    def __init__(self, data_size: int, hidden_size: int, width_size: int, depth: int, *, key: jax.random.PRNGKey, **kwargs):
...
class NeuralCDE(eqx.Module):
    ...
    def __init__(self, data_size: int, hidden_size: int, width_size: int, depth: int, *, key: jax.random.PRNGKey, **kwargs):
        ...
        self.linear = eqx.nn.Linear(hidden_size, 3, key=lkey)  # Output position, velocity, and acceleration
    
    def __call__(self, ts: jnp.ndarray, coeffs: Tuple, evolving_out: bool = False) -> jnp.ndarray:
...
def generate_msd_data_from_full_simulation(dataset_size: int, add_noise: bool, config: Config, *, key: jax.random.PRNGKey) -> Tuple[jnp.ndarray, Tuple, jnp.ndarray, jnp.ndarray, int]:
    ...
    print(f"Generated data shape: ts={ts.shape}, forces={forces.shape}, responses={responses.shape}")
...
def loss_fn(model: NeuralCDE, ts: jnp.ndarray, target_responses: jnp.ndarray, coeffs: Tuple) -> Tuple[float, Dict]:
...
@eqx.filter_jit
def make_step(model: NeuralCDE, data: Tuple, opt_state: Any, optimizer: optax.GradientTransformation) -> Tuple[float, Dict, NeuralCDE, Any]:
    """Single training step."""
    ts, target_responses, *coeffs = data
    
    # Compute loss and gradients
    (loss, metrics), grads = eqx.filter_value_and_grad(loss_fn, has_aux=True)(model, ts, target_responses, coeffs)
...
def train_neural_cde_msd(config: Config) -> Tuple[NeuralCDE, Dict]:
    ...
    print(f"Data shape: ts={ts.shape}, forces={forces.shape}, responses={responses.shape}")
    print(f"Data size: {data_size}")
    print(f"Response dimensions: position, velocity, acceleration")
    
    # Initialize model - update output size to handle 3D state (pos, vel, acc)
    print("Initializing Neural CDE model...")
    model = NeuralCDE(
        data_size, config.hidden_size, config.width_size, config.depth,
        key=model_key
    )
    ...
    # Create dataloader
    dataloader = create_dataloader((ts, responses) + coeffs, config.batch_size, key=loader_key)
    ...
    for step, batch_data in enumerate(dataloader):
        ...
        loss, metrics, model, opt_state = make_step(model, batch_data, opt_state, optimizer)
        step_time = time.time() - start_time
        
        train_losses.append(float(loss))
        train_rmses.append(float(metrics['rmse']))
        
        print(f"Step {step:3d}: Loss={loss:.6f}, RMSE={metrics['rmse']:.6f}, Time={step_time:.3f}s")
    
    # Evaluate on test set
    print("Evaluating on test set...")
    ts_test, coeffs_test, forces_test, responses_test, _ = generate_msd_data_from_full_simulation(
...
def plot_training_history(results: Dict):
    ...
    if config.save_plots:
        plt.savefig(f'msd_training_history.{config.plot_format}', dpi=300, bbox_inches='tight')
        print(f"Training history plot saved as msd_training_history.{config.plot_format}")
...
def plot_msd_predictions(model: NeuralCDE, config: Config):
    ...
    key = jr.PRNGKey(1234)
    ts, coeffs, forces, responses, _ = generate_msd_data_from_full_simulation(4, False, config, key=key)
    ...
    for i in range(min(4, len(ts))):
        ...
        # Plot position
        axes[0, col].plot(ts, responses[i, :, 0], 'b-', linewidth=2, label=f'True Position {i+1}')
        axes[0, col].plot(ts, predictions[i, :, 0], 'r--', linewidth=2, label=f'Predicted Position {i+1}')
        ...
        # Plot velocity
        axes[1, col].plot(ts, responses[i, :, 1], 'g-', linewidth=2, label=f'True Velocity {i+1}')
        axes[1, col].plot(ts, predictions[i, :, 1], 'm--', linewidth=2, label=f'Predicted Velocity {i+1}')
        ...
        # Plot acceleration
        axes[2, col].plot(ts, responses[i, :, 2], 'c-', linewidth=2, label=f'True Acceleration {i+1}')
        axes[2, col].plot(ts, predictions[i, :, 2], 'y--', linewidth=2, label=f'Predicted Acceleration {i+1}')
    ...
    if config.save_plots:
        plt.savefig(f'msd_predictions_3d.{config.plot_format}', dpi=300, bbox_inches='tight')
        print(f"MSD 3D predictions plot saved as msd_predictions_3d.{config.plot_format}")
...
def plot_phase_space_predictions(model: NeuralCDE, config: Config):
    ...
    key = jr.PRNGKey(1234)
    ts, coeffs, forces, responses, _ = generate_msd_data_from_full_simulation(1, False, config, key=key)
    ...
    ax1.plot3D(responses[0, :, 0], responses[0, :, 1], responses[0, :, 2], 'b-', linewidth=2)
    ...
    ax2.plot3D(predictions[0, :, 0], predictions[0, :, 1], predictions[0, :, 2], 'r--', linewidth=2)
    ...
    ax3.plot3D(responses[0, :, 0], responses[0, :, 1], responses[0, :, 2], 'b-', linewidth=2, label='True')
    ax3.plot3D(predictions[0, :, 0], predictions[0, :, 1], predictions[0, :, 2], 'r--', linewidth=2, label='Predicted')
    ...
    if config.save_plots:
        plt.savefig(f'msd_phase_space_predictions.{config.plot_format}', dpi=300, bbox_inches='tight')
        print(f"MSD phase space predictions plot saved as msd_phase_space_predictions.{config.plot_format}")
