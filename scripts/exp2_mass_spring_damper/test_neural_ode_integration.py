...
def test_data_generation(neural_ode_funcs, config, jax):
    ...
    try:
        ...
        ts, train_data, test_data = neural_ode_funcs.generate_synthetic_data(test_config, key=key)
...
def test_training_step(neural_ode_funcs, model, config, jax, jnp, eqx, optax):
    ...
    try:
        ...
        batch = (ts, train_data['initial_conditions'][:batch_size], train_data['trajectories'][:batch_size])
        ...
        print(f"✓ Training step successful: loss={loss:.6f}, rmse={metrics['rmse']:.6f}")
        return True
    except Exception as e:
...
def main():
    ...
    if import_success:
        ...
        if config_success:
            ...
            if test_data_generation(neural_ode_funcs, config, jax):
                ...
                model_success, model = test_model_creation(neural_ode_funcs, config, jax)
                if model_success:
                    tests_passed += 1
                    
                    # Test 5: Forward pass (only if model creation works)
                    if test_model_forward_pass(neural_ode_funcs, model, config, jax, jnp):
                        tests_passed += 1
                    
                    # Test 6: Training step (only if model creation works)
                    if test_training_step(neural_ode_funcs, model, config, jax, jnp, eqx, optax):
                        tests_passed += 1
    ...
    if tests_passed == total_tests:
        print("✓ All tests passed! neural_ode_example.py integration is working correctly.")
        return True
    else:
