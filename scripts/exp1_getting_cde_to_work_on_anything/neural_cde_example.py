...
#%%
# Data Generation
def get_spiral_data(dataset_size: int, add_noise: bool, *, key: jax.random.PRNGKey) -> Tuple[jnp.ndarray, Tuple, jnp.ndarray, int]:
...
def loss_fn(model: NeuralCDE, ts: jnp.ndarray, labels: jnp.ndarray, coeffs: Tuple) -> Tuple[float, Dict]:
...
def make_step(model: NeuralCDE, data: Tuple, opt_state: Any, optimizer: optax.GradientTransformation) -> Tuple[float, Dict, NeuralCDE, Any]:
    ...
    (loss, metrics), grads = eqx.filter_value_and_grad(loss_fn, has_aux=True)(model, ts, labels, coeffs)
...
def train_neural_cde(config: Config) -> Tuple[NeuralCDE, Dict]:
    ...
    # Create dataloader
    dataloader = create_dataloader((ts, labels) + coeffs, config.batch_size, key=loader_key)
    ...
    for step, batch_data in enumerate(dataloader):
        ...
        print(f"Step {step:3d}: Loss={loss:.4f}, Acc={metrics['accuracy']:.4f}, Time={step_time:.3f}s")
...
def plot_training_history(results: Dict):
    ...
    if config.save_plots:
        plt.savefig(f'training_history.{config.plot_format}', dpi=300, bbox_inches='tight')
...
def plot_spiral_predictions(model: NeuralCDE, config: Config):
    ...
    ax.scatter(values[:, 0], values[:, 1], values[:, 2], c=colors, cmap='viridis', 
               s=50, alpha=0.7, label='Prediction')
    
    ax.set_xlabel('Time')
    ax.set_ylabel('X')
    ax.set_zlabel('Y')
    ax.set_title(f'Spiral with Neural CDE Predictions\nTrue Label: {labels[0]:.1f}, Prediction: {predictions[0]:.3f}')
    
    plt.legend()
    plt.tight_layout()
    
    if config.save_plots:
        plt.savefig(f'spiral_predictions.{config.plot_format}', dpi=300, bbox_inches='tight')
        print(f"Spiral predictions plot saved as spiral_predictions.{config.plot_format}")
