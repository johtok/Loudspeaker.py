...
class RealParamsNeuralODEFunc:
    ...
    def __init__(
        ...
        msd_params: Dict[str, float],
        noise_levels: list = [0.1, 0.01, 0.001],
    ):
        """
        ...
        for noise_level in noise_levels:
            noisy_matrix = self._add_noise_to_matrix(
                real_weight_matrix, noise_level, key
            )
    ...
    def _add_noise_to_matrix(
        self, matrix: np.ndarray, noise_level: float, key
    ) -> np.ndarray:
...
def run_exp4():
    ...
    with timeout_context(600):  # 10 minute timeout
        ...
        damping_coefficient = (
            2 * damping_ratio * mass * (2 * jnp.pi * natural_frequency)
        )
        ...
        for noise_level in noise_levels:
            ...
            print(
                f"Weight matrix range: [{noisy_weight_matrix.min():.4f}, {noisy_weight_matrix.max():.4f}]"
            )
            ...
            print(
                f"Initial weight matrix norm: {jnp.linalg.norm(model.func.weight_matrix):.6f}"
            )
            ...
            print(
                f"Error requirement (< 1e-3): {'✓ MET' if error_requirement_met else '✗ NOT MET'}"
            )
            ...
            with open(
                f"{config['visualization']['save_dir']}/exp4_results.pkl", "wb"
            ) as f:
        ...
        for noise_level, results in results_by_noise.items():
            ...
            print(
                f"Noise Level {noise_level:6f}: RMSE={rmse:.8f}, Requirement={'✓' if met_requirement else '✗'}"
            )
