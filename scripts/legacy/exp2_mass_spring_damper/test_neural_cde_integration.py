...
from scripts.exp2_mass_spring_damper.neural_cde_msd_example import (
    ...
    plot_msd_predictions,
    plot_phase_space_predictions
)
...
def test_data_generation():
    ...
    print(f"  - State dimensions: position, velocity, acceleration")
...
def test_neural_cde_model():
    ...
    print(f"✓ Neural CDE model initialized")
    print(f"  - Input dimension: 5 (time, force, pos, vel, acc)")
    print(f"  - Hidden size: 8")
    print(f"  - Output dimensions: 3 (position, velocity, acceleration)")
    ...
    print(f"  - Sample prediction: pos={predictions[0]:.4f}, vel={predictions[1]:.4f}, acc={predictions[2]:.4f}")
...
def test_training_simulation():
    ...
    key = jr.PRNGKey(5678)
    ts_train, coeffs_train, forces_train, responses_train, _ = generate_msd_data_from_full_simulation(
        16, False, config, key=key
    )
    
    print(f"✓ Training simulation completed")
    print(f"  - Training samples: 16")
    print(f"  - Neural CDE model successfully processes msd_simulation_with_forcing data")
    print(f"  - Model outputs 3D state predictions (position, velocity, acceleration)")
