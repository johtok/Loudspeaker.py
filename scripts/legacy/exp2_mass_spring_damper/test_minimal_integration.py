...
def test_core_functionality():
    ...
    try:
        ...
        print(
            f"  Model: hidden_dim={config['model']['hidden_dim']}, output_dim={config['model']['output_dim']}"
        )
        ...
        ts_short = ts[:10]
        predictions = jax.vmap(lambda y0: model(ts_short, y0))(
            train_data["initial_conditions"][:2]
        )
        ...
        print(
            f"✓ Loss function successful: loss={loss:.6f}, rmse={metrics['rmse']:.6f}"
        )
