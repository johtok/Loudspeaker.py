#!/usr/bin/env python3
"""
Exp3: Neural ODE Mass-Spring-Damper Experiment (50 samples)
===========================================================

This experiment is identical to Exp2 but with 50 samples instead of 5.
...
def run_exp3():
    """Run Exp3 neural ODE experiment with 50 samples."""
    
    with timeout_context(600):  # 10 minute timeout
        print("Starting Exp3: Neural ODE Mass-Spring-Damper Experiment (50 samples)")
        print("=" * 60)
        ...
        # Exp3 Configuration - same as Exp2 but with 50 samples
        config = create_neural_ode_config(
            ...
            batch_size=10,   # Larger batch size for 50 samples
            weight_decay=1e-4,
            optimizer='adam',
        ...
        print(f"  MSD natural frequency: {config['msd_params']['natural_frequency']} Hz")
        print(f"  Forcing bandwidth: {config['forcing']['forcing_frequency_range']} Hz")
        print(f"  Model parameters: 6 (2x3 matrix without bias)")
        ...
        print("\nGenerating synthetic data with 50 samples...")
        key = jr.PRNGKey(42)
        ...
        # Save results
        with open(f"{config['visualization']['save_dir']}/exp3_results.pkl", 'wb') as f:
        ...
        # Final summary
        print("\n" + "=" * 60)
        print("EXP3 NEURAL ODE EXPERIMENT COMPLETED")
        print("=" * 60)
...
if __name__ == "__main__":
    try:
        model, history, results = run_exp3()
        if results and results['metrics']['error_requirement_met']:
            print("\n🎉 Exp3 SUCCESS: Error requirement achieved!")
        else:
            print("\n⚠️  Exp3 PARTIAL: Training completed but error requirement not met")
