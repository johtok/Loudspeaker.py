...
class DataGenerationTests:
    ...
    def test_synthetic_data_generation(self):
        ...
        assert train_data['initial_conditions'].shape == (int(train_size), expected_output_dim)
        assert train_data['trajectories'].shape == (int(train_size), expected_seq_len, expected_output_dim)
        assert train_data['ts'].shape == ts.shape
        
        # Validate test data
        test_size = config['data']['dataset_size'] * config['data']['test_split']
        assert test_data['initial_conditions'].shape == (int(test_size), expected_output_dim)
        assert test_data['trajectories'].shape == (int(test_size), expected_seq_len, expected_output_dim)
    ...
    def test_data_generation_with_forcing(self):
        ...
        assert test_data['forcing'].shape == (int(config['data']['test_split'] * config['data']['dataset_size']), expected_seq_len)
    ...
    def test_msd_data_generation(self):
        ...
        try:
            ...
            assert train_data['trajectories'].shape[-1] == 3  # position, velocity, acceleration
            ...
            print("⚠️ MSD simulation module not available, skipping MSD data generation test")
...
class ModelArchitectureTests:
    ...
    def test_neural_ode_model_creation(self):
        ...
        assert solution.shape == (10, 3), f"Expected shape (10, 3), got {solution.shape}"
...
class TrainingTests:
    ...
    def test_single_training_step(self):
        ...
        # Create batch
        batch = (ts, train_data['initial_conditions'][:4], train_data['trajectories'][:4])
        ...
        param_changes = [jnp.sum((new - old)**2) for old, new in zip(old_params, new_params) if hasattr(new, 'shape')]
    ...
    def test_training_loop(self):
        ...
        assert final_loss <= initial_loss * 2, "Loss should not increase dramatically"  # Allow some increase due to noise
    ...
    def test_different_optimizers(self):
        ...
        for optimizer_name in optimizers_to_test:
            ...
            # Create model
            model_key = jr.fold_in(self.suite.test_key, hash(optimizer_name + 100) % 1000)
            model = NeuralODEModel(
                data_size=3,
                hidden_dim=16,
                num_layers=2,
    ...
    def test_batch_processing(self):
        ...
        dataloader = create_dataloader(train_data, config['training']['batch_size'], key=dataloader_key)
...
class SolvingTests:
    ...
    def test_stiff_vs_non_stiff_solvers(self):
        ...
        non_stiff_solution = solve_neural_ode(model, ts, y0, {'solver': non_stiff_config})
...
class EvaluationTests:
    ...
    def test_metric_computation(self):
        ...
        expected_metrics = ['total_mse', 'total_rmse', 'relative_error', 'max_error', 'r2_score', 'dim_mse', 'dim_rmse']
    ...
    def test_per_dimension_metrics(self):
        ...
        # Get predictions
        predictions = jax.vmap(lambda y0: solve_neural_ode(model, ts, y0, config))(test_data['initial_conditions'])
    ...
    def test_differentiability_testing(self):
        ...
        if diff_test['gradients_computable']:
            assert diff_test['gradients_finite'], "Gradients should be finite for a well-initialized model"
...
class VisualizationTests:
    ...
    def test_visualization_disabled(self):
        ...
        try:
            ...
            raise AssertionError(f"Visualization functions should handle disabled state gracefully: {e}")
    ...
    def test_visualization_enabled(self):
        ...
        try:
            ...
            if "matplotlib" in str(e).lower() or "display" in str(e).lower():
                ...
                raise AssertionError(f"Visualization functions should work when enabled: {e}")
...
class IntegrationTests:
    ...
    def test_error_handling_and_edge_cases(self):
        ...
        try:
            ...
            test_config['training']['num_steps'] = 5
            trained_model, history = train_neural_ode(model, train_data, test_config, test_data)
    ...
    def test_memory_efficiency(self):
        ...
        dataset_sizes = [16, 32, 64] if self.suite.config.quick_mode else [16, 32, 64, 128]
...
class PerformanceTests:
    ...
    def test_data_generation_performance(self):
        ...
        dataset_sizes = [32, 64, 128] if self.suite.config.quick_mode else [32, 64, 128, 256]
        ...
        for dataset_size in dataset_sizes:
            ...
            assert generation_time < 30, f"Data generation took too long: {generation_time}s"
    ...
    def test_model_forward_pass_performance(self):
        ...
        for seq_len in sequence_lengths:
            for batch_size in batch_sizes:
                ...
                if self.suite.config.verbose:
                    print(f"  Seq len {seq_len}, batch {batch_size}: {forward_time:.3f}s")
    ...
    def test_training_performance(self):
        ...
        if self.suite.config.verbose:
            print(f"  Training: {training_time:.3f}s, loss improvement: {loss_improvement:.2%}")
...
def run_all_tests(config: TestConfig = None):
    ...
    all_tests = [
        ...
        (visualization_tests.test_visualization_config_defaults, "Visualization Config Defaults"),
        
        # Integration tests
        (integration_tests.test_complete_pipeline, "Complete Pipeline"),
        (integration_tests.test_different_parameter_combinations, "Parameter Combinations"),
        ...
        # Performance tests
        (performance_tests.test_data_generation_performance, "Data Generation Performance"),
        (performance_tests.test_model_forward_pass_performance, "Forward Pass Performance"),
        (performance_tests.test_training_performance, "Training Performance"),
        (performance_tests.test_solver_performance_comparison, "Solver Performance Comparison"),
    ]
...
def main():
    ...
    parser.add_argument('--quick-mode', action='store_true', help='Use smaller datasets for faster testing')
    parser.add_argument('--save-plots', action='store_true', help='Save plots (if enabled in config)')
    parser.add_argument('--raise-on-error', action='store_true', help='Raise exceptions on test failures')
    parser.add_argument('--category', action='append', dest='categories',
                       choices=['configuration', 'data', 'model', 'training', 'solving',
                               'evaluation', 'visualization', 'integration', 'performance'],
