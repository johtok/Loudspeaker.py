...
print(
    f"  Model: hidden_dim={config['model']['hidden_dim']}, num_layers={config['model']['num_layers']}"
)
print(
    f"  Data: dataset_size={config['data']['dataset_size']}, simulation_time={config['data']['simulation_time']}"
)
print(
    f"  Training: num_steps={config['training']['num_steps']}, learning_rate={config['training']['learning_rate']}"
)
...
print(
    f"Model created with {sum(p.size for p in jax.tree_util.tree_leaves(model) if p.ndim > 0)} parameters"
)
...
# Train the model
trained_model, training_history = train_neural_ode(model, train_data, config, test_data)
...
print(
    f"Model parameters: {sum(p.size for p in jax.tree_util.tree_leaves(trained_model) if p.ndim > 0)}"
)
