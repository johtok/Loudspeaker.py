...
from scripts.exp2_mass_spring_damper.msd_simulation_with_forcing import ForcingType
...
def generate_msd_data_for_cde(
    dataset_size: int, add_noise: bool, simulation_time: float, *, key
):
    ...
    if add_noise:
        ...
        state_responses = (
            state_responses + jr.normal(noise_key, state_responses.shape) * 0.01
        )

    # Compute interpolation coefficients for the control path
    coeffs = jax.vmap(diffrax.backward_hermite_coefficients)(
        control_data[..., 0], control_data
    )
...
class NeuralCDE(eqx.Module):
    ...
    def __init__(
        self, control_size: int, hidden_size: int, width_size: int, depth: int, *, key
    ):
        ikey, fkey, lkey = jr.split(key, 3)

        self.initial = eqx.nn.MLP(
            control_size, hidden_size, width_size, depth, key=ikey
        )
        self.func = eqx.nn.MLP(
            hidden_size + control_size,
            hidden_size,
            width_size,
            depth,
...
def train_neural_cde_msd():
    ...
    for step in range(num_steps):
        ...
        if step % 10 == 0 or step == num_steps - 1:
            print(
                f"Step {step:3d}: Loss={loss:.6f}, RMSE={metrics['rmse']:.6f}, "
...
def plot_results(model, results):
    ...
    for i in range(min(3, len(predictions))):
        ...
        # Position
        axes[row, col].plot(
            ts, state_responses[i, :, 0], "b-", linewidth=2, label="True Position"
        )
        axes[row, col].plot(
            ts, predictions[i, :, 0], "r--", linewidth=2, label="Predicted Position"
        )
        axes[row, col].plot(
            ts, state_responses[i, :, 1], "g-", linewidth=2, label="True Velocity"
        )
        axes[row, col].plot(
            ts, predictions[i, :, 1], "m--", linewidth=2, label="Predicted Velocity"
        )
        axes[row, col].plot(
            ts, state_responses[i, :, 2], "c-", linewidth=2, label="True Acceleration"
        )
        axes[row, col].plot(
            ts, predictions[i, :, 2], "y--", linewidth=2, label="Predicted Acceleration"
        )
