...
class NeuralODEModel(eqx.Module):
    ...
    def __call__(
        self, ts: jnp.ndarray, y0: jnp.ndarray, solver_config: Dict[str, Any] = None
    ) -> jnp.ndarray:
        """
...
# %%
# Data Generation
def generate_pink_noise_bandpassed(
    length: int,
    sample_rate: float,
    ...
    pink_spectrum = pink_spectrum.at[mask].set(
        1.0 / jnp.sqrt(jnp.abs(frequencies[mask]))
    )
...
def generate_synthetic_data(
    config: Dict[str, Any], *, key: jax.random.PRNGKey
...
def _generate_generic_synthetic_data(
    config: Dict[str, Any], key: jax.random.PRNGKey
    ...
    train_data = {
        ...
        "forcing": (
            forcing_signals[:train_size] if forcing_signals is not None else None
        ),
    }
...
def _generate_msd_data_with_forcing_exp2(
    config: Dict[str, Any], key: jax.random.PRNGKey
) -> Tuple[jnp.ndarray, Dict[str, Any], Dict[str, Any]]:
    """
    ...
    for i in range(dataset_size):
        ...
        def msd_ode(t, y, args):
            ...
            return jnp.array(
                [vel, new_acc, 0.0]
            )  # acceleration derivative is 0 for simplicity
...
def _generate_forcing_signals(
    ...
    for i in range(batch_size):
        ...
        if forcing_config["type"] == "pink_noise":
            ...
            f_center = (
                forcing_config["frequency_range"][0]
                + forcing_config["frequency_range"][1]
            ) / 2
            ...
            signal = forcing_config["amplitude"] * jnp.sin(
                2 * math.pi * f0 * t + math.pi * (f1 - f0) * t**2
            )
...
def _generate_trajectories(
    ...
    config: Dict[str, Any],
    forcing_signals: jnp.ndarray,
    key: jax.random.PRNGKey,
) -> jnp.ndarray:
    """Generate trajectories using a known ODE system."""

    def true_dynamics(
        t: float, y: jnp.ndarray, forcing_val: float, output_dim: int
    ) -> jnp.ndarray:
    ...
    for i, (y0, forcing_signal) in enumerate(
        zip(
            initial_conditions,
            (
                forcing_signals
                if forcing_signals is not None
                else [None] * len(initial_conditions)
            ),
        )
    ):
        ...
        def ode_func(t, y, args):
            forcing_val = (
                forcing_interp.evaluate(t) if forcing_interp is not None else 0.0
            )
...
def prepare_neural_ode_data(
    ...
    if forcing is not None:
        ...
        input_data = jnp.concatenate(
            [time_expanded, forcing_expanded, trajectories], axis=-1
        )
    else:
...
def loss_fn_neural_ode(
    ...
    # Relative error
    rel_error = jnp.mean(
        jnp.abs(target_batch - predictions) / (jnp.abs(target_batch) + 1e-8)
    )
...
def loss_fn_neural_ode_with_timeout(
    model: NeuralODEModel,
    ts: jnp.ndarray,
    y0_batch: jnp.ndarray,
    target_batch: jnp.ndarray,
...
def train_step(
    ...
    solver_config: Dict[str, Any] = None,
) -> Tuple[jnp.ndarray, Dict[str, jnp.ndarray], NeuralODEModel, Any]:
    """
...
def train_neural_ode(
...
) -> Tuple[NeuralODEModel, Dict[str, Any]]:
    """
    ...
    if train_config["weight_decay"] > 0:
        ...
        optimizer = optax.chain(
            optimizer, optax.add_decayed_weights(train_config["weight_decay"])
        )
    ...
    for step in range(train_config["num_steps"]):
        ...
        batch_indices = jr.choice(
            jr.PRNGKey(step), indices, (train_config["batch_size"],), replace=False
        )
        ...
        if test_data is not None and step % eval_config["eval_frequency"] == 0:
            ...
            if eval_config["early_stopping"]:
                if test_loss < history["best_loss"]:
                    ...
                    history["best_model"] = jax.tree_util.tree_map(
                        lambda x: x.copy() if hasattr(x, "copy") else x, model
                    )
        ...
        if step % 100 == 0 or step == train_config["num_steps"] - 1:
            print(
                f"Step {step:4d}: Loss={loss:.6f}, RMSE={metrics['rmse']:.6f}, "
...
def solve_neural_ode(
    ...
    solver_config = config.get(
        "solver",
        {"solver": diffrax.Tsit5(), "rtol": 1e-3, "atol": 1e-6, "adaptive_steps": True},
    )

    return model(ts, y0, solver_config)


def solve_with_forcing(
    model: NeuralODEModel,
    ts: jnp.ndarray,
    y0: jnp.ndarray,
    forcing_signal: jnp.ndarray,
    config: Dict[str, Any] = None,
) -> jnp.ndarray:
    """
...
def evaluate_model(
    model: NeuralODEModel, test_data: Dict[str, Any], config: Dict[str, Any]
) -> Dict[str, float]:
    """
...
def evaluate_model_step(
    model: NeuralODEModel, test_data: Dict[str, Any], solver_config: Dict[str, Any]
) -> Tuple[jnp.ndarray, Dict[str, jnp.ndarray]]:
    """
...
def differentiability_test(
    model: NeuralODEModel, ts: jnp.ndarray, y0: jnp.ndarray, config: Dict[str, Any]
) -> Dict[str, bool]:
    """
...
def plot_training_history(history: Dict[str, Any], config: Dict[str, Any]) -> None:
    ...
    if history.get("test_loss"):
        eval_steps = range(
            ...
            config.get("evaluation", {}).get("eval_frequency", 100),
        )
        axes[0, 0].plot(
            eval_steps[: len(history["test_loss"])],
            history["test_loss"],
            "r-",
            linewidth=2,
            label="Test Loss",
    ...
    if history.get("test_rmse"):
        eval_steps = range(
            0,
            len(history["train_rmse"]),
            config.get("evaluation", {}).get("eval_frequency", 100),
        )
        axes[0, 1].plot(
            eval_steps[: len(history["test_rmse"])],
            history["test_rmse"],
            "m-",
            linewidth=2,
            label="Test RMSE",
...
def plot_trajectories(
    ...
    for i in range(min(num_samples, len(y0_batch))):
        for dim in range(min(3, target_batch.shape[2])):  # Plot up to 3 dimensions
            axes[i, dim].plot(
                ts, target_batch[i, :, dim], "b-", linewidth=2, label=f"True Dim {dim}"
            )
            axes[i, dim].plot(
                ts,
                predictions[i, :, dim],
...
def create_dataloader(
    data: Dict[str, Any], batch_size: int, *, key: jax.random.PRNGKey
) -> Callable:
    """
