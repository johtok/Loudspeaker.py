...
from scripts.exp2_mass_spring_damper.msd_simulation_with_forcing import (
    MSDConfig, ForcingType, SolverType, NormalizationType,
    run_single_simulation, run_batch_simulation, 
...
def demo_basic_simulation():
    ...
    print(f"Configuration: Natural freq={config.natural_frequency}Hz, "
...
def demo_batch_simulation():
    ...
    successful = sum(1 for stats in batch_result['stats'] if stats.get('successful', False))
    ...
    if successful > 0:
        ...
        print(f"  - Max displacements range: {min(max_displacements):.6f} to {max(max_displacements):.6f}m")
...
def demo_parameter_sweep():
    ...
    print(f"Testing {len(frequencies)} frequencies × {len(damping_ratios)} damping ratios")
...
def main():
    ...
    try:
        ...
        from scripts.exp2_mass_spring_damper.msd_simulation_with_forcing import MSDSimulator
