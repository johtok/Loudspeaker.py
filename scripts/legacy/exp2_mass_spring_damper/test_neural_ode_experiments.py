...
from neural_ode_funcs import (
    ...
    evaluate_model_step,
    generate_pink_noise_bandpassed,
...
class TestNeuralODEExperiments(unittest.TestCase):
    ...
    def test_neural_ode_func_creation(self):
        ...
        func = NeuralODEFunc(
            data_size=3, hidden_dim=3, num_layers=1, activation="tanh", key=func_key
        )
    ...
    def test_data_generation(self):
        ...
        ts, train_data, test_data = generate_synthetic_data(
            self.minimal_config, key=self.key
        )
        ...
        print(
            f"Generated data: ts={ts.shape}, train={train_data['trajectories'].shape}"
        )
    ...
    def test_pink_noise_generation(self):
        ...
        pink_noise = generate_pink_noise_bandpassed(
            length, sample_rate, freq_range, key
        )
        ...
        passband_mask = (jnp.abs(frequencies) >= freq_range[0]) & (
            jnp.abs(frequencies) <= freq_range[1]
        )
        ...
        print(
            f"Pink noise generated: {pink_noise.shape}, energy ratio: {passband_energy / total_energy:.3f}"
        )
    ...
    def test_loss_function(self):
        ...
        model = NeuralODEModel(data_size=3, hidden_dim=3, num_layers=1, key=model_key)
        ...
        loss, metrics = evaluate_model_step(
            model,
            {"ts": ts, "initial_conditions": y0_batch, "trajectories": target_batch},
            {},
        )
    ...
    def test_training_step(self):
        ...
        loss, metrics, new_model, new_opt_state = train_step(
            model, optimizer, opt_state, batch
        )
        ...
        self.assertTrue(
            params_changed, "Model parameters should change after training step"
        )
    ...
    def test_integration_exp2_config(self):
        ...
        print(
            f"Exp2 config verified: {exp2_config['data']['dataset_size']} samples, {exp2_config['data']['sample_rate']} Hz"
        )
    ...
    def test_integration_exp3_config(self):
        ...
        self.assertEqual(
            train_data["trajectories"].shape[0], 40
        )  # 50 samples, 80% train
        ...
        print(
            f"Exp3 config verified: {exp3_config['data']['dataset_size']} samples, {exp3_config['data']['sample_rate']} Hz"
        )
    ...
    def test_error_requirement_check(self):
        ...
        targets = jnp.array(
            [[0.1001, 0.2001, 0.3001], [0.4001, 0.5001, 0.6001]]
        )  # Small error
        ...
        print(
            f"Error requirement test - RMSE: {rmse:.8f}, Requirement met: {rmse < 1e-3}"
        )
    ...
    def test_reproducibility(self):
        ...
        key1 = jr.PRNGKey(42)
        ts1, train_data1, test_data1 = generate_synthetic_data(
            self.minimal_config, key=key1
        )

        # Run second experiment with same seed
        key2 = jr.PRNGKey(42)
        ts2, train_data2, test_data2 = generate_synthetic_data(
            self.minimal_config, key=key2
        )

        # Results should be identical
        self.assertTrue(jnp.allclose(ts1, ts2))
        self.assertTrue(
            jnp.allclose(train_data1["trajectories"], train_data2["trajectories"])
        )
        self.assertTrue(
            jnp.allclose(test_data1["trajectories"], test_data2["trajectories"])
        )
