...
def test_data_generation(config: Dict[str, Any], key: jr.PRNGKey):
    ...
    try:
        ...
        print(
            f"  Actual train initial_conditions shape: {train_data['initial_conditions'].shape}"
        )
        print(f"  Actual train trajectories shape: {train_data['trajectories'].shape}")
        print(
            f"  Actual test initial_conditions shape: {test_data['initial_conditions'].shape}"
        )
        print(f"  Actual test trajectories shape: {test_data['trajectories'].shape}")

        # Check that sizes add up correctly
        assert (
            actual_train_size + actual_test_size == expected_total_size
        ), f"Train + test size ({actual_train_size} + {actual_test_size}) != total size ({expected_total_size})"
    ...
    try:
        ...
        for metric in expected_metrics:
            if metric in metrics:
                ...
                assert hasattr(value, "item") or isinstance(
                    value, (float, int, jnp.number)
                )
            else:
...
def test_solving_functions(model, test_data: Dict[str, Any], config: Dict[str, Any]):
    ...
    try:
        ...
        solution_custom = solve_neural_ode(
            model, ts, y0, {"solver": custom_solver_config}
        )
...
def test_utility_functions(config: Dict[str, Any], key: jr.PRNGKey):
    ...
    try:
        ...
        dummy_data = {
            ...
            "forcing": (
                jnp.ones((10, len(ts))) if config["forcing"]["enabled"] else None
            ),
        }
...
def main():
    ...
    try:
        ...
        if trained_model is not None:
            metrics, detailed_metrics = test_evaluation_functions(
                trained_model, test_data, config
            )
